...
import com.rongchat.scanner.camera.CameraManager;
import com.rongchat.scanner.decoding.CaptureActivityHandler;
import com.rongchat.scanner.decoding.InactivityTimer;
import com.rongchat.scanner.view.ViewfinderView;
    ...
    public void handleDecode(com.google.zxing.Result result, Bitmap barcode) {
        ...
        if (!isNoCute) {
            if (TextUtils.isEmpty(resultString)) {
                NToast.shortToast(CaptureActivity.this, "Scan failed!");
                return;
            } else {
                if (resultString.startsWith("RCfriend@")) {
                    String[] s = resultString.split("@");
                    String friendId = s[1];
                    NToast.shortToast(CaptureActivity.this, "对方的id为:" + friendId);
                    ...
                    NToast.shortToast(this, "扫描结果为：" + result);
                }
            }
        }
        finish();
